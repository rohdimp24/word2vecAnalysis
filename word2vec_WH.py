...
sentenceIterator = gensim.models.word2vec.LineSentence("/Users/305015992/pythonProjects/word2vecAnalysis/WHDesc.txt")
...
findSimilar(model,"desup")

'''Dump the result in the file'''
import json
fp=open('output_word2vec_meridium_WH.json','w')
...
tt=tsne.fit_transform(word_vectors)
import matplotlib.pyplot as plt

vectors = np.asfarray(word_vectors, dtype='float')

vocab.index("desup")
vocab.index("desuperheater")
...
findSimilar(model,"turbine")
##tests
v1=model['pump']+model['turbine']
v2=model['pump']+model['wheelspace']

v1-v2

#http://stackoverflow.com/questions/30795944/how-can-a-sentence-or-a-document-be-converted-to-a-vector